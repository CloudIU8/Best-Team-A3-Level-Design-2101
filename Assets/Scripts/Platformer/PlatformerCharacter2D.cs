...
namespace UnityStandardAssets._2D
{
    public class PlatformerCharacter2D : MonoBehaviour
    {
        ...
        public void Move(float move, bool crouch, bool jump, bool sustainJump, bool dash) // Note:this function was updated from the original Move function to add a fifth argument to recognise the DASH. This is called from PlatformerCharacter2DUserControl (Physics based example project).
        {
                ...
                switch(m_ApplyMoveForce)
                {
                    case ApplyMoveForce.Instantaneous:
                    {
                        ...
                        if (!m_Grounded)
                        {
                                ...
                                m_Rigidbody2D.linearVelocity = new Vector2(move * m_AirMaxSpeed * m_s_SScale * m_curDashSpeedMultiplier, m_Rigidbody2D.linearVelocity.y);
                            }
                        else
                        {
                                m_Rigidbody2D.linearVelocity = new Vector2(move * m_MaxSpeed * m_s_SScale * m_curDashSpeedMultiplier, m_Rigidbody2D.linearVelocity.y);
                            }
                        break;
                    }
                    case ApplyMoveForce.Additive:
                    {
                        ...
                        if (!m_Grounded)
                        {
                            ...
                            if (m_Rigidbody2D.linearVelocity.magnitude < (m_AirMaxSpeed * m_s_SScale))
                            {
                        ...
                        {
                            // Move the character using addforce to produce an acceleration curve, but only if max speed has not been reached, reduce this if slimed.
                            if (m_Rigidbody2D.linearVelocity.magnitude < (m_MaxSpeed * m_s_SScale))
                            {
                                m_Rigidbody2D.AddForce(new Vector2(move * accel, 0f));
                            }
                        }
            ...
            else if (!m_Grounded && m_MidAirJump && jump)
            {
                if (m_JumpCount < m_MidAirJumpCount)
                {
                    ...
                    m_Grounded = false;
                    m_Anim.SetBool("Ground", false);
                    m_Rigidbody2D.linearVelocity = new Vector2(m_Rigidbody2D.linearVelocity.x, 0f); // Reset velocity on jump...
            ...
            if (!m_Grounded && m_SustainedJump && sustainJump)
            {
                ...
                if (m_SustainedJumpTimer > 0)
                {
                    ...
                    {
                        if (m_Rigidbody2D.linearVelocity.y < (m_MaxSustainedJumpSpeed * m_s_SScale))
                        {
        ...
        private void DefaultRigidBodySettings()
        {
            ...
            m_defaultDrag = m_Rigidbody2D.linearDamping;
            m_defaultAngularDrag = m_Rigidbody2D.angularDamping;
        }
        ...
        public void Slimed()
        {
            ...
            if (!m_Slimed)
            {
                ...
                m_Rigidbody2D.linearDamping *= m_SlimeDragScale;
                m_Rigidbody2D.angularDamping *= m_SlimeAngularDragScale;
        ...
        public void WashSlime()
        {
            ...
            m_Rigidbody2D.linearDamping = m_defaultDrag;
            m_Rigidbody2D.angularDamping = m_defaultAngularDrag;
